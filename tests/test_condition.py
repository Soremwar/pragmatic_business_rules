...
class TestEvaluateCondition:
	def test_variable_exists(self):
		variable = "xyz"
		with pytest.raises(Exception, match=f"Variable '{variable}' not defined") as e:
			evaluate_condition({"name": variable,"operator":"equal_to", "value": "something"}, {})

	def test_variable_matches_type(self):
		variable_name = "abc"
		variable_value = 1
		condition_value = "a string"
		with pytest.raises(Exception, match="The value '{}' to compare for variable '{}' doesn't match the defined variable type of '{}'".format(
			condition_value,
			variable_name,
			type(variable_value).__name__,
		)) as e:
			evaluate_condition({"name": variable_name,"operator":"equal_to", "value": condition_value}, {variable_name: variable_value})